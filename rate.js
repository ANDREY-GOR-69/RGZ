...
// Получить актуальный курс INR
async function loadCurrentRate() {
  const res = await fetch("https://www.cbr-xml-daily.ru/daily_json.js");
  const data = await res.json();
  const rate = data.Valute.INR.Value;
  currentRate = rate;

  document.getElementById('result').textContent = `Курс INR сегодня: ${rate.toFixed(2)} ₽`;
  setupConverter(rate);
}
...
function setupConverter(rate) {
  ...
  rubInput.addEventListener('input', () => {
    if (rubInput.value)
      inrInput.value = (parseFloat(rubInput.value) / rate).toFixed(2);
    else inrInput.value = '';
  });

  inrInput.addEventListener('input', () => {
    if (inrInput.value)
      rubInput.value = (parseFloat(inrInput.value) * rate).toFixed(2);
    else rubInput.value = '';
  });
}
